  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/companies/get-company?role=${"manager"}`,
        {
          method: "GET",
          headers: {
          ...
          query: JSON.stringify({ role: "manager" }),
        }
      );

      if (!response.ok) {
        return thunkAPI.rejectWithValue(
          data.errors || data.message || "Error fetching company data"
        );
      }
...
export const fetchEmployees = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3001/companies/get-employees?role=${"manager"}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
